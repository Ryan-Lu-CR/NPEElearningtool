...
describe('English Part B bank data', () => {
  ...
  it('stores every English resource under its default bank folder', () => {
    const resources = banks.flatMap(bank => bank.chapters.flatMap(chapter => chapter.sections.flatMap(section => [
    ...
    expect(new Set(resources.map(({ url }) => url)).size).toBeGreaterThanOrEqual(504)
    resources.forEach(({ bank, url }) => {
      expect(url).toMatch(/^\/api\/default-workspace\/file\?path=/)
      const relative = new URL(url, 'http://localhost').searchParams.get('path') || ''
      expect(relative.startsWith(`英语一真题/${bank.name}/资源/`)).toBe(true)
    })
  })
  ...
  it('includes all eight ordering paragraphs in years with fixed paragraphs', () => {
    for (const year of [2023, 2025, 2026]) {
      const section = sections.find(({ bank }) => bank.id === `english-${year}`)?.section
